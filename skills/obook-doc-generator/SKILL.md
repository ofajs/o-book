...
在 Markdown 文件中使用 `<o-playground>` 标签时，**标签中间绝对不能有空行**（空白换行），否则会导致解析错误。
...
|------|------|------|
| `path` | 是 | 指定文件路径和文件名 |
| `preview` | 否 | 标记为预览文件，组件会在预览区加载此文件。**⚠️ 只能用于普通模板（不带 `page` 或 `component` 属性）** |
| `active` | 否 | 设置为当前激活状态，默认在编辑器中显示此文件 |
| `unimportant` | 否 | 标记为"不重要"文件，可通过设置隐藏此类文件 |

**⚠️ 重要限制 - `preview` 属性使用规则**：

- ✅ **可以用于**：普通模板（`<template>` 不带 `page` 或 `component` 属性）
- ❌ **不能用于**：Page 模块（`<template page>`）或组件模块（`<template component>`）

**原因**：Page 模块和组件模块需要通过 ofa.js 的特定机制加载，不能直接作为预览文件。只有普通模板可以直接在预览区运行。
...
组件会自动识别以下文件类型：
- **Page模块**：`<template page>`
- **组件模块**：`<template component>`
- **普通模板**：`<template>` 或 `<pre>` 或其他标签
- **其他文件**：无特殊标记

**⚠️ 重要说明 - `<template>` 标签的处理方式**：

1. **普通模板**（不带 `page` 或 `component` 属性）：
   ```markdown
   <template>
     <div>Hello World</div>
   ...
   ```
   - 相当于**去掉 `<template>` 标签**，直接将内容放到 HTML 文件中
   - playground 会自动注入 ofa.js 环境
   - 适合快速演示简单的 HTML/JS 代码
...
2. **Page 模块**（带 `page` 属性）：
   ...
   ```
   - 作为 **ofa.js 的 Page 模块**处理
   - 保留 `<template page>` 标签，写入虚拟文件
   - 适合演示 ofa.js 页面功能

3. **组件模块**（带 `component` 属性）：
   ```markdown
   <template component>
   ...
   ```
   - 作为 **ofa.js 的组件模块**处理
   - 保留 `<template component>` 标签，写入虚拟文件
   - 适合演示 ofa.js 组件功能
...
**⚠️ 注意事项**：
- 在 Markdown 文件中使用时，**标签内不能有任何空行**
- 所有 `<code>` 标签必须紧跟在 `<o-playground>` 标签后，中间不能有空行
- 每个 `</code>` 和下一个 `<code>` 之间不能有空行
- 最后一个 `</code>` 和 `</o-playground>` 之间不能有空行
- **重要**：playground 虚拟环境已自动加载 ofa.js，**不需要在代码中重复引用 ofa.js**
- **重要**：`preview` 属性只能用于普通模板，**不能用于 Page 模块或组件模块**
